...
- [x] マッチ名複数行表示に伴うテーブル表示崩れの修正 (StoreTable.tsxの改善)
- [x] PC版テーブルへの縦スクロールバー追加 (max-height制限とヘッダー固定の最適化)
- [x] スクロールバーのカスタムスタイリング適用 (index.cssの更新)
- [x] 「未実施」ステータスの優先表示ロジック実装 (PC/スマホ共通)
- [x] Context Bridgeスキルの再読み込みとドキュメント確認
